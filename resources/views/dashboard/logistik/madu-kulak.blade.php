    ...
    <div
        ...
        <div class="mt-2 ml-2 relative overflow-x-auto shadow-md sm:rounded-lg">
            <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                ...
                <tbody>
                    @foreach ($madukulakPaginate as $madukulakPaginates)
                    <tr
                        class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                        <th scope="row" class="px-4 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                            {{ $madukulakPaginates->nama_madu }}
                        </th>
                        <td class="px-6 py-4">
                            {{ $madukulakPaginates->size }}
                        </td>
                        <td class="px-6 py-4">
                            {{ 'Rp ' . number_format ($madukulakPaginates->harga_per_gram)}}
                        </td>
                        <td class="px-6 py-4">
                            {{ 'Rp ' . number_format ($madukulakPaginates->harga_total)}}
                        </td>
                        <td class="px-6 py-4">
                            {{ date('d M Y', strtotime($madukulakPaginates->created_at)) }}
                        </td>
                        <td class="px-8">
                            <form class=""
                                action="{{ route('maduKulak.destroy', ['maduKulak' => $madukulakPaginates->id]) }}"
                                method="POST">
                                @csrf
                                @method('delete')
