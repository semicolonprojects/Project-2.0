...
<div class="mt-20 ml-10">
    <table
        ...
        <tr>
            <th class="px-10 py-5">{{ $showFirst->order_id }}</th>
        </tr>
        <tr class="bg-white border">
            ...
            @if ($showFirst->status_pembayaran == 'Termin')
        ...
        <tr>
            <td class="px-10 ">{{ $shows->customer->nama_lengkap }}</td>
            <td class="px-6">{{ $shows->produk->nama_barang }}</td>
            <td class="px-6">{{ $shows->total_order }}</td>
            <td class="px-6">{{ 'Rp ' . number_format($shows->total_pembelian, 0, ',', '.') }}</td>
            <td class="px-6">{{ $shows->channel->nama_channel }}</td>
            <td class="px-6">{{ $shows->tipe_pembayaran }}</td>
            <td class="px-6">{{ $shows->status_pembayaran }}</td>
            @if ($shows->status_pembayaran == 'Termin')
            <td>{{ $shows->tenggat_order }}</td>
            @endif
            <td class="px-6">{{ $shows->status_barang }}</td>
            <td class="px-6">{{ 'Rp ' . number_format($shows->ongkir, 0, ',', '.') }}</td>
            <td class="px-6"><button id="defaultModalButtonEdit{{ $shows->id }}"
                    data-modal-toggle="defaultModalEdit{{ $shows->id }}">
                ...
                <form action="{{ route('orderCurah.destroy', ['orderCurah' => $shows->id]) }}" method="post"
                    class='inline'>
        ...
        <tr class="border">
            ...
            <td colspan="6" class="font-bold pl-[270px] py-5 text-[#2E2E2E] ">{{ number_format($showFirst->diskon, 2)
                }}%
            </td>
        </tr>
        <tr class="border">
            <td colspan="6" class="px-5 py-5">Closed By</td>
            <td colspan="6" class="font-bold pl-[270px] py-5 text-[#2E2E2E] ">{{ $showFirst->user->username }}</td>
        </tr>
        <tr class="border">
            <td colspan="6" class="px-5 py-5">Note</td>
            <td colspan="6" class="font-bold pl-[270px] py-5 text-[#2E2E2E] ">{{ $showFirst->note }}</td>
        </tr>
    </table>
</div>

@foreach ($show as $modals)
<!-- Edit modal -->
<div id="defaultModalEdit{{ $modals->id }}" tabindex="-1" aria-hidden="true"
        ...
        <div class="relative p-4 bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
            ...
            <div class="flex justify-between items-center pb-4 mb-4 rounded-t border-b sm:mb-5 dark:border-gray-600">
                <h3 class="text-lg font-semibold text-gray-900 dark:text-white">
                    Edit Order {{ $showFirst->order_id }}
                </h3>
                <button type="button"
                    class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center dark:hover:bg-gray-600 dark:hover:text-white"
                    data-modal-toggle="defaultModalEdit{{ $modals->id }}">
            ...
            <!-- Modal body -->
            <form action="{{ route('orderCurah.update', ['orderCurah'=>$modals->id]) }}" method="POST">
                @csrf
                @method('put')
                <div class="grid gap-4 mb-4 sm:grid-cols-2">
                    <div>
                        ...
                        <select
                            ...
                            name="kode_barang">
                            <option selected value="{{ $modals->kode_barang }}">{{ $modals->produk->nama_barang }}
                            </option>
                    ...
                    <div>
                        ...
                        <input type="text"
                            ...
                            value="{{ $modals->total_order}}" name="total_order">
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <select
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                            name="tipe_pesanan">
                            <option selected value="{{ $modals->tipe_pesanan }}">{{ $modals->channel->nama_channel }}
                            </option>
                    ...
                    <div>
                        ...
                        <select id="pesanan"
                            ...
                            <option selected>{{ $modals->status_pembayaran }}</option>
                    ...
                    <div id="terminFields" style="display: none;">
                        <label for="total_termin"
                            ...
                            Termin</label>
                        <input id="total_termin" type="number" name="total_termin" value="{{ $modals->total_termin }}"
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">

                        <label for="tenggat_order"
                            class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Tenggat
                            Order</label>
                        <input id="tenggat_order" type="date" name="tenggat_order" value="{{ $modals->tenggat_order }}"
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                    </div>
                    <div>
                        ...
                        <select id="pesanan"
                            ...
                            <option selected>{{ $modals->tipe_pembayaran }}</option>
                    ...
                    <div>
                        ...
                        <select
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                            name="status_barang">
                            <option selected>{{ $modals->status_barang }}</option>
                    ...
                    <div>
                        ...
                        <input type="number" name="ongkir"
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                            required value="{{ $modals->ongkir }}">
                        <input hidden name="created" value="{{ $modals->created_at }}">
                    </div>
                </div>
                <button type="submit"
